...
export const createLocalStorageContainer = function <T extends ZodType<unknown, ZodTypeDef, unknown>>(
  ...
  return new (class StorageStore {
    ...
    static generateData(storeData: StateType): string {
     ...
     getStoreData() {
      return (
        JSON.parse(
          localStorage.getItem(storeName) ?? StorageStore.generateData({})
     ...
     setItem<T extends StateKeyUnion>(key: T, value: StateType[T]) {
      ...
      try {
        ...
        localStorage.setItem(storeName, StorageStore.generateData(draftData));
      } catch (error) {
