...
import { ExtractEffects, ExtractState, GetState, SetState, UnSubscribe } from './shared';
import {
...
export interface DefineModel<TContext> {
  <
    TStateCreator extends ModelStateTemplate<ContainerDependencies<TContext, TDynamic>>,
    TDynamic extends boolean = false,
  >(
    stateCreator: TStateCreator,
    isDynamic?: TDynamic
  ): DefineModelOutput<ModelStructureData<ExtractState<TStateCreator>, {}>, TContext, TDynamic>;
}
/**
 * @name 定义动态Model对外暴露方法的接口
 */
type DynamicModelApi<TState, TDependencies, TStructureData extends ModelStructureData> = {
  bindActions: <TActions extends ActionsOutputTemplate>(
    actions: ModelActionsTemplate<TActions, TState>
  ) => DefineModelOutput<
    ModelStructureData<TState, TStructureData['actions'] & ExtractEffects<NoInfer<TActions>>>,
    TDependencies,
    true
  >;
  /**注入阶段 同时去实例化对象 */
 ...
 */
type StaticModelApi<TState, TDependencies, TStructureData extends ModelStructureData> = {
  get: GetState<TState>;
  set: SetState<TState>;
  bindActions: <TActions extends ActionsOutputTemplate>(
    actions: ModelActionsTemplate<TActions, TState>
